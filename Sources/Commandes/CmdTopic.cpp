...
{
    std::cout << SGREEN << "~   Topic a été utilisé par l'utilisateur n°" << _id << END << std::endl;
    if (msg->param.empty())
    ...
    // std::cout << GGREEN << msg->param.size() << END << std::endl;
    // else if (!this_channel->checkOperator(this))
